...
def forward(X, w1, w2): #Forward propagate helps calculate y_hat - Used during classification
    h = sigmoid(np.matmul(prepend_bias(X), w1))
    y_hat = softmax(np.matmul(prepend_bias(h), w2))
    return (y_hat, h)

def back(X, Y, y_hat, w2, h): #Back propagate helps calculate gradient - Used during training to calculate gradient for gradient descent
...
if __name__ == "__main__":
    ...
    target_folder_path = os.path.abspath("../mnist_digit_classifier")
    sys.path.insert(0, target_folder_path)


    import mnist
    w1, w2 = train(mnist.X_train, mnist.Y_train, mnist.X_test, mnist.Y_test, n_hidden_nodes=200, epochs = 2, batch_size = 128, lr=0.01)